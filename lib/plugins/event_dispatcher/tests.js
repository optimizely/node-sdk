...
describe('lib/plugins/event_dispatcher', function() {
  describe('APIs', function() {
    describe('dispatchEvent', function() {
      ...
      beforeEach(function() {
        sinon.stub(callbackObj, 'callback');
        nock('https://cdn.com')
          .post('/event')
          .reply(200, {
            ok: true,
      ...
      it('should execute the callback passed to event dispatcher', function(done) {
        ...
        eventDispatcher.dispatchEvent(eventObj, callbackObj.callback)
        .on('response', function(response) {
          done();
          sinon.assert.calledOnce(callbackObj.callback);
        })
        .on('error', function(error) {
          assert.fail('status code okay', 'status code not okay', "")
        })
