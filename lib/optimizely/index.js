...
function Optimizely(config) {
  ...
  this.dispatchEvent = function (eventToDispatch, callback) {
    var eventDispatcherResponse = this.eventDispatcher.dispatchEvent(eventToDispatch, callback);
    if (eventDispatcherResponse && eventDispatcherResponse.hasOwnProperty('then') && typeof eventDispatcherResponse.then == 'function') {
      eventDispatcherResponse.then(callback);
    }
  }
...
Optimizely.prototype.activate = function(experimentKey, userId, attributes) {
  ...
  try {
    ...
    this.dispatchEvent(impressionEvent, eventDispatcherCallback);
...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventTags) {
  ...
  try {
    ...
    this.dispatchEvent(conversionEvent, eventDispatcherCallback);
  } catch (ex) {
    this.logger.log(LOG_LEVEL.ERROR, ex.message);
...
/**
 * Dispatches an event and executes the designated callback if the dispatch returns a callback
 * @param  eventToDispatch
 * @param  callback
 * @return {null}
 */
 // function dispatchEvent (eventToDispatch, callback) {
 //   var eventDispatcherResponse = this.eventDispatcher.dispatchEvent(eventToDispatch, callback);
 //   if (eventDispatcherResponse && eventDispatcherResponse.hasOwnProperty('then') && typeof eventDispatcherResponse.then == 'function') {
 //     eventDispatcherResponse.then(callback);
 //   }
