...
Optimizely.prototype.activate = function(experimentKey, userId, attributes) {
  try {
    ...
    impressionEventOptions.configObj = this.configObj;
    impressionEventOptions.experimentKey = experimentKey;
    impressionEventOptions.variationId = variationId;
    impressionEventOptions.userId = userId;
    impressionEventOptions.attributes = attributes;

    var impressionEvent = eventBuilder.getImpressionEvent(impressionEventOptions);
    var dispatchedImpressionEventLogMessage = sprintf(LOG_MESSAGES.DISPATCH_IMPRESSION_EVENT, MODULE_NAME, impressionEvent.url, impressionEvent.params);
...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventValue) {
  try {
    ...
    conversionEventOptions.configObj = this.configObj,
    conversionEventOptions.eventKey = eventKey;
    conversionEventOptions.userId = userId;
    conversionEventOptions.attributes = attributes;
    conversionEventOptions.eventValue = eventValue;
    conversionEventOptions.variationIds = variationIds;
    conversionEventOptions.validExperimentIdsForGoal = validExperimentIdsForGoal;
    console.log(validExperimentIdsForGoal)

    var conversionEvent = eventBuilder.getConversionEvent(conversionEventOptions);

    var dispatchedConversionEventLogMessage = sprintf(LOG_MESSAGES.DISPATCH_CONVERSION_EVENT, MODULE_NAME, conversionEvent.url, conversionEvent.params);
