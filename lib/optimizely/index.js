...
function Optimizely(config) {
  ...
  if (!config.datafile) {
    ...
    if (config.skipJSONValidation === true) {
      ...
      try {
        var correctProjectConfigSchema = config.datafile.version === NEW_OPTIMIZELY_VERSION ?
          projectConfigSchema.V2 : projectConfigSchema.V1;
        if (jsonSchemaValidator.validate(correctProjectConfigSchema, config.datafile)) {
          this.configObj = projectConfig.createProjectConfig(config.datafile);
          this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.VALID_DATAFILE, MODULE_NAME));
        }
      } catch (ex) {
...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventValue) {
  try {
    ...
    var validExperimentIdsForEvent = this.__getValidExperimentIdsForEvent(eventKey, userId, attributes);
    if (!validExperimentIdsForEvent.length) {
      var noValidExperimentsForEventToTrack = sprintf(LOG_MESSAGES.NO_VALID_EXPERIMENTS_FOR_EVENT_TO_TRACK, MODULE_NAME, eventKey);
 ...
 * @return {Array<string>}
 */
Optimizely.prototype.__getValidExperimentIdsForEvent = function(eventKey, userId, attributes) {
  ...
  return validExperimentIdsForEvent;
};
