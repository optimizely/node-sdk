...
Optimizely.prototype.activate = function(experimentKey, userId, attributes) {
  try {
    ...
    var variationId;
    if (this.__returnForcedVariationIfProvided(experimentKey, userId)) {
      variationId = this.__returnForcedVariationIfProvided(experimentKey, userId);
    } else {
...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventValue) {
  try {
    ...
    var validExperimentKeysForEvent = this.__getValidExperimentKeysForEvent(eventKey, userId, attributes);
    ...
    var variationIds = this.__getBucketedVariationIdsForUser(validExperimentKeysForEvent, userId);
 ...
 * @return {Object<Array>}     Object consisting of valid experiment keys for event
 */
Optimizely.prototype.__getValidExperimentKeysForEvent = function(eventKey, userId, attributes) {
  ...
  return validExperimentKeysForEvent;
};
 ...
 * @return {Array<string>}
 */
Optimizely.prototype.__getBucketedVariationIdsForUser = function(validExperimentKeys, userId) {
  try {
    var variationIds = _.map(validExperimentKeys, function(experimentKey) {
      if (this.__returnForcedVariationIfProvided(experimentKey, userId)) {
        return this.__returnForcedVariationIfProvided(experimentKey, userId);
      } else {
        var bucketerParams = this.__buildBucketerParams(experimentKey, userId);
 ...
 */
Optimizely.prototype.__returnForcedVariationIfProvided = function(experimentKey, userId) {
