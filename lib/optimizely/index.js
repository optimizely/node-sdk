...
Optimizely.prototype.activate = function(experimentKey, userId, attributes, sessionId) {
  ...
  if (typeof sessionId !== 'undefined' && !!sessionId) {
    this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.DEPRECATED_SESSION_ID, MODULE_NAME, 'activate'))
  }
 ...
 * @param  {string} sessionId: ID to identify the user's current session
 */
Optimizely.prototype.track = function(eventKey, userId, attributes, eventValue, sessionId, eventTags) {
  if (!this.isValidInstance) {
  ...
  if (typeof sessionId !== 'undefined' && !!sessionId) {
    this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.DEPRECATED_SESSION_ID, MODULE_NAME, 'track'))
  }
  ...
  try {
    ...
    if (!eventTags && (sessionId || eventValue)) {
      eventTags = {};
    }

    if (sessionId) {
      eventTags['sessionId'] = sessionId;
    }

    if (eventValue) {
      eventTags['eventValue'] = eventValue;
    }
