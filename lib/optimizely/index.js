...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventTags) {
  ...
  try {
    ...
    var conversionEventOptions = {
      ...
      eventTags: eventTags,
      userId: userId,
      experimentsToBucketedVariations: validExperimentsToBucketedVariations
    };
 ...
 * @param  {string} userId
 * @param  {Object} attributes
 * @return {Object} Map of experiments that we want to track to variations keys in which the user has been bucketed
 */
Optimizely.prototype.__getValidExperimentsForEvent = function(eventKey, userId, attributes) {
  ...
  validExperimentsToVariationsMap = _.reduce(experimentIdsForEvent, function(results, experimentId) {
    ...
    if (variationKey) {
      ...
      if (!projectConfig.isRunning(this.configObj, experimentKey)) {
        ...
        results[experimentKey] = variationKey;
      }
    } else {
