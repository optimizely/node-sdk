...
Optimizely.prototype.activate = function(experimentKey, userId, attributes) {
  ...
  try {
    ...
    var eventDispatcherCallback = function() {
     ...
     var eventDispatcherResponse = this.eventDispatcher.dispatchEvent(impressionEvent, eventDispatcherCallback);

     //if eventDispatcherResponse is a Promise, execute our callback
     if (eventDispatcherResponse && eventDispatcherResponse.hasOwnProperty('then') && typeof eventDispatcherResponse.then == 'function') {
       eventDispatcherResponse.then(eventDispatcherCallback);
     }
...
Optimizely.prototype.track = function(eventKey, userId, attributes, eventTags) {
  ...
  try {
    ...
    var eventDispatcherResponse = this.eventDispatcher.dispatchEvent(conversionEvent, eventDispatcherCallback);
    if (eventDispatcherResponse && eventDispatcherResponse.hasOwnProperty('then') && typeof eventDispatcherResponse.then == 'function') {
      eventDispatcherResponse.then(eventDispatcherCallback);
    }
