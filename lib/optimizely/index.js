 ...
 * @param  {string}      sessionId: ID to identify the user's current session
 * @return {string|null}
 */
Optimizely.prototype.activate = function(experimentKey, userId, attributes, sessionId) {
  ...
  try {
    if (!this.__validateInputs(userId, attributes) || !this.__checkIfExperimentIsRunningOrLaunched(experimentKey, userId)) {
    ...
    if (!projectConfig.shouldDispatchEvent(this.configObj, experimentKey)) {
 ...
 * @param  {string} sessionId: ID to identify the user's current session
 */
Optimizely.prototype.track = function(eventKey, userId, attributes, eventValue, sessionId) {
  if (!this.isValidInstance) {
...
Optimizely.prototype.__getValidExperimentInformationForEvent = function(eventKey, userId, attributes) {
  ...
  if (this.configObj.eventKeyMap[eventKey]) {
    ...
    _.forEach(this.configObj.eventKeyMap[eventKey].experimentIds, function(experimentId) {
      ...
      if (!this.__checkIfExperimentIsRunningOrLaunched(experimentKey, userId)) {
 ...
 */
Optimizely.prototype.__checkIfExperimentIsRunningOrLaunched = function(experimentKey, userId) {
  if (!projectConfig.isExperimentRunningOrLaunched(this.configObj, experimentKey)) {
