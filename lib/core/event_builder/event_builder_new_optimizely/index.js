...
var SESSION_ID_EVENT_TAG_NAME = 'sessionId';
...
module.exports = {
  ...
  getConversionEventParams: function(configObj, eventKey, eventTags, variationIds, validExperimentKeysForEvent) {
    ...
    if (eventTags && eventTags['eventValue']) {
      var revenueMetric = {
        name: REVENUE_EVENT_METRIC_NAME,
        value: eventTags['eventValue'],
      };
      conversionEventParams.eventMetrics.push(revenueMetric);
    }
    if (eventTags && eventTags['sessionId']) {
      var sessionIdFeature = {
        id: SESSION_ID_EVENT_TAG_NAME,
        type: this.CUSTOM_ATTRIBUTE_FEATURE_TYPE,
        value: eventTags['sessionId'],
        shouldIndex: false,
      };
      conversionEventParams.eventFeatures.push(sessionIdFeature);
    }
