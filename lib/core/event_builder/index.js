 ...
 * @param  {string} projectId ID of project for which event is being sent
 * @return {string} URL for the Classic Optimizely event API
 */
function getClassicOptimizelyEndpoint(projectId) {
  return sprintf('https://%s.log.optimizely.com/event', projectId);
}
...
var NEW_OPTIMIZELY_CONVERSION_ENDPOINT = 'https://log.dz.optimizely.com/log/event';
...
module.exports = {
  ...
  getImpressionEvent: function(options) {
    ...
    if (options.configObj.version === NEW_OPTIMIZELY_VERSION) {
      ...
      impressionEvent.url = getClassicOptimizelyEndpoint(options.configObj.projectId);
  ...
  getConversionEvent: function(options) {
    ...
    if (options.configObj.version === NEW_OPTIMIZELY_VERSION) {
      ...
      conversionEvent.url = getClassicOptimizelyEndpoint(options.configObj.projectId);
