...
module.exports = {
  ...
  getConversionEvent: function(options) {
    ...
    if (options.configObj.version === NEW_OPTIMIZELY_VERSION) {
      ...
      var conversionEventParams = newOptimizelyEventBuilder.getConversionEventParams(options.configObj, options.eventKey, options.eventValue, options.variationIds, options.validExperimentKeysForEvent);
      conversionEvent.params = _.assignIn(commonParams, conversionEventParams);
    } else {
      conversionEvent.url = sprintf(classicOptimizelyEventBuilder.CLASSIC_OPTIMIZELY_ENDPOINT, options.configObj.projectId);

      var conversionEventParams = classicOptimizelyEventBuilder.getConversionGoalParams(options.configObj, options.eventKey, options.eventValue);
      var experimentVariationParams = classicOptimizelyEventBuilder.getExperimentVariationParams(options.configObj, options.variationIds, options.validExperimentKeysForEvent);
