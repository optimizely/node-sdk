...
var EVENT_METRIC_NAME = 'revenue';

function getCommonParamsV2(configObj, userId, attributes) {
  var commonParams = {};
  commonParams.accountId = configObj.accountId;
  commonParams.projectId = configObj.projectId;
  commonParams.visitorId = userId;
  commonParams.timestamp = Math.round(new Date().getTime() / 1000.0);
  commonParams.isGlobalHoldback = false;
  commonParams.userFeatures = [];
...
function getImpressionEventParams(configObj, experimentKey, variationId) {
  ...
  impressionEventParams.layerId = projectConfig.getLayerId(configObj, experimentKey);
  impressionEventParams.decision = {
...
function getConversionEventParams(configObj, eventKey, eventValue, variationIds, validExperimentIdsForGoal) {
  var conversionEventParams = {};
  conversionEventParams.eventEntityId = projectConfig.getEventId(configObj, eventKey);
  conversionEventParams.eventName = eventKey;
  conversionEventParams.eventFeatures = [];
  conversionEventParams.layerStates = [];
  ...
  conversionEventParams.eventMetrics = []; // revenue event
  if (eventValue) {
    var revenueMetric = {
...
function getUrl(configObj) {
  if (configObj.version === NEW_OPTIMIZELY_VERSION) {
    return 'https://p13nlog.dz.optimizely.com/log/decision';
  } else {
...
module.exports = {
  ...
  getImpressionEvent: function(options) {
    ...
    if (options.configObj.version === NEW_OPTIMIZELY_VERSION) {
      var commonParams = getCommonParamsV2(options.configObj, options.userId, options.attributes);
