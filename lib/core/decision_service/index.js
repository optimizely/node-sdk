...
DecisionService.prototype.getVariation = function(experimentKey, userId, attributes) {
  ...
  variation = this.__getStoredVariation(experiment, userId);
  if (!!variation) {
  ...
  // persist bucketing
  this.__storeVariation(experiment, variation, userId);
  return variation.key;
};
 ...
 * @param  {Object} experiment
 * @param  {string} userId
 * @return {Object} the stored variation or null if there isn't one
 */
DecisionService.prototype.__getStoredVariation = function(experiment, userId) {
  if (!this.userProfileService) {
    return null;
  }

  try {
    var userProfile = this.userProfileService.lookup(userId);
    if (userProfile && userProfile.decisions.hasOwnProperty(experiment.id)) {
      var experimentDecision = userProfile.decisions[experiment.id];
      variationId = experimentDecision.variation_id;
      return this.configObj.variationIdMap[variationId];
    }
  } catch (ex) {
 ...
 * @param  {Object} experiment experiment for which we are storing the decision
 * @param  {Object} variation  the variation the user is bucketed into for the given experiment
 * @param  {string} userId     the ID of hte user
 */
DecisionService.prototype.__storeVariation = function(experiment, variation, userId) {
  if (!this.userProfileService) {
    return;
  }
  ...
  try {
    ...
    userProfile.decisions[experiment.id] = {
