...
DecisionService.prototype.getVariation = function(experimentKey, userId, attributes) {
  ...
  if (!!userProfile && userProfile.decisions.hasOwnProperty(experiment.id)) {
    var decision = userProfile.decisions[experiment.id];
    var variationId = decision.variation_id;
    if (this.configObj.variationIdMap.hasOwnProperty(variationId)) {
      variation = this.configObj.variationIdMap[decision.variation_id];
    }
  }
  if (!!variation) {
 ...
 * @param  {string} userId
 * @return {Object} the stored user profile or null if there isn't one
 */
