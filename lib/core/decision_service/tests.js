    ...
    describe('#getVariation', function() {
      ...
      describe('when a user profile service is provided', function() {
        ...
        it('should return the previously bucketed variation', function() {
          ...
          sinon.assert.notCalled(bucketerStub);
          assert.strictEqual(mockLogger.log.args[0][1], 'DECISION_SERVICE: Returning previously activated variation \"control\" of experiment \"testExperiment\" for user \"decision_service_user\" from user profile.');
        });
        ...
        it('should re-bucket if the stored variation is no longer valid', function() {
          ...
          sinon.assert.calledWith(userProfileLookupStub, 'decision_service_user');
          sinon.assert.calledOnce(bucketerStub);
          assert.strictEqual(mockLogger.log.args[0][1], 'DECISION_SERVICE: User decision_service_user was previously bucketed into variation with ID not valid variation for experiment testExperiment, but no matching variation was found.');
        });
        ...
        it('should store the bucketed variation for the user', function() {
          ...
          userProfileLookupStub.returns({
            user_id: 'decision_service_user',
            experimentBucketMap: {}, // no decisions for user
          });
          ...
          assert.strictEqual(mockLogger.log.args[0][1], 'DECISION_SERVICE: Saved variation "control" of experiment "testExperiment" for user "decision_service_user".');
        });
        ...
        it('should log an error message if "lookup" throws an error', function() {
          ...
          assert.strictEqual(mockLogger.log.args[0][1], 'DECISION_SERVICE: Error while looking up user profile for user ID "decision_service_user": I am an error.');
        });
        ...
        it('should log an error message if "save" throws an error', function() {
          ...
          sinon.assert.calledWith(userProfileLookupStub, 'decision_service_user');
          sinon.assert.calledOnce(bucketerStub); // should still go through with bucketing
          assert.strictEqual(mockLogger.log.args[0][1], 'DECISION_SERVICE: Error while saving user profile for user ID "decision_service_user": I am an error.');
        });
      });
    });
