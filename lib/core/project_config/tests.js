...
describe('lib/core/project_config', function() {
  ...
  describe('projectConfig helper methods', function() {
    ...
    it('should return true if experiment status is set to Running or Launch in isActive', function() {
      assert.isTrue(projectConfig.isActive(configObjNewOptly, 'testExperiment'));

      assert.isTrue(projectConfig.isActive(configObjNewOptly, 'testExperimentLaunched'));
    });

    it('should return true if experiment status is set to Running or Launch in isActive', function() {
      assert.isFalse(projectConfig.isActive(configObjNewOptly, 'testExperimentNotRunning'));
    });

    it('should return true if experiment status is set to Running in isRunning', function() {
      assert.isTrue(projectConfig.isRunning(configObjNewOptly, 'testExperiment'));
    });

    it('should return false if experiment status is not set to Running in isRunning', function() {
      assert.isFalse(projectConfig.isRunning(configObjNewOptly, 'testExperimentLaunched'));
    });
